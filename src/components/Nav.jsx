    ...
    return (
        <div className='w-full h-[70px] bg-[#ff6146] fixed flex justify-between pl-8 pr-4 z-20'>
            <div className='h-full flex items-center'>
                <Link to={`/pizzadimama/Home`} onClick={()=> {scrollToTop(); openCart == true ? dispatch(cartToggle()) : "" }} className='text-white font-bold text-[20px]'>Pizza di Mama</Link>
            </div>
            <div className='flex items-center gap-2 group'>
                <Link to={`/pizzadimama/MakeYourPizza`} onClick={()=> {scrollToTop(); openCart == true ? dispatch(cartToggle()) : ""}} className={` bg-yellow-300 font-bold px-6 py-2 rounded-md transition-all hover:brightness-[95%]`}>Custom</Link>
